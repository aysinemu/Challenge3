...
function connectWebSocket() {
    ...
    if (message.toLowerCase().includes("done") && coordResolve) {
        console.log("Received DONE confirmation");
        coordResolve();
        coordResolve = null;
        
        if (manualFieldset.classList.contains('manual-waiting')) {
            manualFieldset.classList.remove('waiting', 'manual-waiting');
        }
        if (coordFieldset.classList.contains('coord-waiting')) {
            coordFieldset.classList.remove('waiting', 'coord-waiting');
        }
        if (testFieldset.classList.contains('test-waiting')) {
...
async function moveManual(dx) {
    ...
    manualX += dx;
    manualX = Math.max(0, Math.min(670, manualX));
...
async function handleCoordinateSubmit(e) {
    ...
    if (x < 0 || x > 670) {
    rangeError.textContent = 'X must be between 0 and 670.';
    return;
    }
...
function generateTestXCoords(count = 20, min = 0, max = 670, minDistance = 400) {
    const coords = [];
